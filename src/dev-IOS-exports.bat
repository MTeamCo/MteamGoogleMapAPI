...
rem IOS export V
%adt% -package -target %ios_targ% -keystore "%ios_certificate%" -storetype pkcs12 -storepass %ios_pass%  -provisioning-profile  "%ios_mobprevision%"  "%name%-dev.ipa" "%name%-app-ios-dev.xml"  "%name%.swf"  %contents% %ios_contents% %native_folder%
